...
interface VideoListProps {
    ...
    categories: string[];
    onEditVideo?: (article: Article) => void;
}
...
const VideoList: React.FC<VideoListProps> = ({ videos, onAddVideo, onUpdateVideo, onDeleteVideos, categories, onEditVideo }) => {
    ...
    const handleBulkDelete = () => {
        if (window.confirm(`Are you sure you want to delete ${selectedItems.length} selected videos?`)) {
            onDeleteVideos(selectedItems);
            setSelectedItems([]);
    ...
    return (
        <div className="w-full">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8">
                <div>
                    <h2 className="text-3xl font-bold text-brand-dark-blue">Manage Videos</h2>
                    <p className="text-gray-600 mt-1">Add, edit, or delete videos.</p>
                </div>
            </div>
