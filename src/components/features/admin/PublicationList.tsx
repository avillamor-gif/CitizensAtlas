...
const PublicationList: React.FC<PublicationListProps> = ({ publications, onAddPublication, onUpdatePublication, onDeletePublications, publicationTypes, publicationCategories, onAddPublicationCategory, onEditPublication }) => {
    ...
    const handleDelete = (articleId: number) => {
        if (window.confirm('Are you sure you want to delete this publication?')) {
            onDeletePublications([articleId]);
    ...
    const handleBulkDelete = () => {
        if (window.confirm(`Are you sure you want to delete ${selectedItems.length} selected publications?`)) {
            onDeletePublications(selectedItems);
            setSelectedItems([]);
    ...
    return (
        <div className="w-full">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8">
                <div>
                    <h2 className="text-3xl font-bold text-brand-dark-blue">Manage Publications</h2>
                    <p className="text-gray-600 mt-1">Add, edit, or delete publications.</p>
                </div>
            </div>
            
             {selectedItems.length > 0 && (
                <div className="flex items-center justify-between p-4 bg-blue-100 border border-blue-200 rounded-lg mb-4">
                    <p className="text-sm font-medium text-blue-800">{selectedItems.length} publication(s) selected.</p>
                    <button
            ...
            <div className="mb-4">
                <div className="relative">
                    <input
                        type="text"
                        placeholder="Search by title, publication type, or slug..."
                            ...
                            <tr>
                                <th scope="col" className="px-6 py-4">
                                     <Checkbox
                                        ...
                                        aria-label="Select all publications on this page"
                                    />
                                </th>
                                <SortableTableHeader label="Title" sortKey="title" sortConfig={sortConfig} requestSort={requestSort} />
                                <SortableTableHeader label="Publication Type" sortKey="category" sortConfig={sortConfig} requestSort={requestSort} />
